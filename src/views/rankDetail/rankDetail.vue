  ...
  <transition-view>
    <div class="full_header">
      <title-back
        ...
        :transition="true"
        :transition-h="infoH"
        :transitiont-title="$route.params.name"
      />
    </div>
    <div class="full_main">
      <div class="container">
        <loading :is-loading="tracks.length" />
          ...
          <div class="datilImg">
            <div class="Mask">
              <div ref="info" class="info">
                ...
                <p class="count">
                  <i class="iconfont icon-mayi-erji-jianbian" />{{
                    listDetail.playCount | playCount_filter
                  }}
                  万
                </p>
              </div>
            </div>
            <img :src="`${listDetail.coverImgUrl}?param=400y400`" alt="" />
          </div>
          <div class="datilList">
            <div class="total">
              <span @click="fn_playAll"><i class="iconfont icon-bofang" /></span>
              <p>播放全部（共{{ tracks.length }}首）</p>
            </div>
            <ul class="list">
              <li
...
export default {
  ...
  methods: {
    ...
    async fn_getRecomDetal() {
      try {
        const {
          data: {
            code,
            playlist: { name, playCount, tracks, coverImgUrl, createTime, tags }
          }
        } = await this.$http({
          url: '/top/list',
    ...
    //全部播放
    async fn_playAll() {
      const Musics = this.tracks.map(item => {
        const {
          id,
          al: { picUrl },
          ar: artists,
      ...
      const playlistID = this.tracks.map(({ id, name }) => {
        return {
          id,
          name
        }
